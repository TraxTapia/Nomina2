...
namespace Nomina.ClienteRest.Client
{
    public class Cliente
    {
        ...
        public GetListCompaniasResponseDTO GetSelectCompanias(string _RouteApi)
        {
            var Response = new GetListCompaniasResponseDTO();
            //if (!this.HasToken())
            //{
            //    Response.Result.SetStatusCode(OperationResult.StatusCodesEnum.UNAUTHORIZED);
            //    Response.Result.AddException(new Exception("No se encontro el Token de Autorización."));
            ...
            Message _MessageFactory = new Message();
            Response = _MessageFactory.SendRequest<GetListCompaniasResponseDTO>(this.UrlEndPoint, _RouteApi, string.Empty, HttpMethod.Post);
            return Response;
        }
        public SelctGeneralResponseDTO GetCategoria(string _RouteApi)
        {
            var Response = new SelctGeneralResponseDTO();
            if (!this.ExisteEndPoint())
            {
                Response.Result.SetStatusCode(StatusCodesEnum.NOT_FOUND);
                Response.Result.AddException(new Exception("Es necesario asignar una UrlEndPoint."));
        ...
        public SelctGeneralResponseDTO GetSelectGral(string _RouteApi)
        {
            var Response = new SelctGeneralResponseDTO();
            //if (!this.HasToken())
            //{
